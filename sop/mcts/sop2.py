...
def e_greedy_rollout(
    ...
    while sim_indices.numel() > 0:
        ...
        if s_greedy_i.numel() > 0:
            m = mask[choose_greedy]
            c = current_nodes[s_greedy_i]
            scores = heuristic[b_greedy_i, c]
            action = torch.argmax(torch.masked_fill(scores, m, 0), dim=-1)
